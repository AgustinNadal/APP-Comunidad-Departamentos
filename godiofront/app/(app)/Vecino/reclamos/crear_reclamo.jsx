...
export default function crear_reclamo() {
  ...
  const sendStoredReclamos = async () => {
    ...
    for (const reclamo of reclamosGuardados) {
      try {
        const response = await axios.post(`http://192.168.83.213:8080/inicio/reclamo?documento=${documento}&idsitio=${sitio}&iddesperfecto=${desperfecto}&descripcion=${descripcion}`, reclamo);
  ...
  const handleCrearReclamo = async () => {
    const reclamo = await axios.post(`http://192.168.83.213:8080/inicio/reclamo?documento=${documento}&idsitio=${sitio}&iddesperfecto=${desperfecto}&descripcion=${descripcion}`,{
      documento,
      sitio,
      desperfecto,
      descripcion,
    ...
    try {
      const response = await axios.post(`http://192.168.83.213:8080/inicio/reclamo`, reclamo);
